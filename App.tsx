...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (bgLayerRef.current) bgLayerRef.current.style.transform = `translate(${-targetX * 10}px, ${-targetY * 10}px) scale(1.1)`;
      ...
      if (titleRef.current) {
        titleRef.current.style.transform = `translate(${targetX * 10}px, ${targetY * 10}px) translateZ(400px)`;
      }
  ...
  return (
    <div className={`fixed inset-0 w-full flex flex-col items-center justify-between overflow-hidden perspective-[2000px] ${currentTheme.appBg} ${currentTheme.isDark ? 'text-white' : 'text-slate-900'}`}>
      ...
      <div ref={floatLayerRef} className="fixed inset-0 pointer-events-none z-0 overflow-hidden">
         {bubbles.map((b, i) => (
          <div key={i} className="bubble opacity-20" style={{ left: b.left, width: b.size, height: b.size, animationDuration: b.duration, animationDelay: b.delay, background: currentTheme.isDark ? 'white' : '#64748b' }} />
        ))}
      </div>

      {/* Header Controls */}
      <header className="w-full flex justify-between items-start relative z-[5000] shrink-0 pointer-events-none pt-4 px-4">
        <div className="flex flex-col gap-2 p-2 rounded-[2rem] bg-black/60 backdrop-blur-xl border border-white/10 shadow-3xl pointer-events-auto">
        ...
        <div className="flex flex-col items-end gap-2 pointer-events-auto">
          ...
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-black/80 backdrop-blur-lg border border-white/10 shadow-xl tray-inset">
      ...
      >
        <h1 className="text-3xl sm:text-4xl md:text-5xl font-black tracking-tighter text-white drop-shadow-[0_10px_20px_rgba(0,0,0,0.8)] leading-none italic uppercase select-none">
          Brainvita<span className={currentTheme.isDark ? "text-cyan-400" : "text-fuchsia-500"}>3D</span>
        </h1>
        <div className="inline-flex items-center gap-2 px-6 py-1.5 rounded-full bg-black/70 backdrop-blur-xl mt-3 border border-white/10 shadow-2xl pointer-events-auto">
          <span className="text-[10px] font-black uppercase text-white/40 tracking-[0.3em]">Remaining</span>
          <span className="text-xl font-black text-white">{marblesRemaining}</span>
        </div>
      </div>

      {/* Main Game Area - Dynamic Board Scaling */}
      <main className="flex-1 w-full flex justify-center items-center relative z-[3000] overflow-visible perspective-[1500px] min-h-0">
         <div className="scale-[0.42] xs:scale-[0.45] sm:scale-55 md:scale-65 lg:scale-80 origin-center transition-transform duration-500">
      ...
      {/* Footer Area - Optimized for Visibility */}
      <footer className="w-full max-w-xl flex flex-col gap-4 relative z-[4500] shrink-0 px-4 pb-4 pointer-events-auto items-center">
        <div className="flex justify-center gap-4 w-full">
          ...
          <button onClick={startGame} className="btn-3d w-40 h-12">
            ...
            <div className="btn-surface bg-blue-600 border-t border-blue-400 rounded-2xl flex items-center justify-center gap-2">
              ...
              <span className="text-white text-sm font-black uppercase tracking-widest">Restart</span>
            </div>
          </button>
        </div>
      ...
      {showThemeModal && (
        <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 bg-black/95 backdrop-blur-3xl animate-in">
          <div className="relative max-w-3xl w-full p-8 rounded-[3rem] shadow-4xl overflow-hidden max-h-[80vh] flex flex-col border border-white/10 bg-slate-950 text-white">
             ...
             <div className="grid grid-cols-2 sm:grid-cols-3 gap-6 overflow-y-auto p-2 custom-scrollbar">
                {THEMES.map(t => (
                   <button key={t.name} onClick={() => handleThemeChange(t)} className="btn-3d h-44 group">
                        <div className={`btn-edge rounded-[2rem] ${currentTheme.name === t.name ? 'bg-green-800' : 'bg-black'}`}></div>
                        <div className={`btn-surface flex flex-col rounded-[2rem] overflow-hidden border-2 ${currentTheme.name === t.name ? 'border-green-400 ring-4 ring-green-500/20' : 'border-white/5 bg-slate-900'}`}>
                            ...
                            <div className="p-3 w-full bg-black/90 flex justify-between items-center border-t border-white/5">
                              ...
                              {currentTheme.name === t.name && <Check size={16} className="text-green-400" strokeWidth={5} />}
                            </div>
                        </div>
                   </button>
                ))}
      ...
      {showRules && (
        <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 bg-black/98 backdrop-blur-3xl animate-in">
           <div className="relative max-w-sm w-full p-10 rounded-[4rem] bg-slate-950 border border-white/10 text-white shadow-4xl">
              <h2 className="text-3xl font-black mb-8 uppercase italic tracking-tighter">Rulebook</h2>
              <div className="space-y-6 text-base font-bold text-slate-400">
                <div className="flex gap-4 items-start">
                  <div className="w-10 h-10 rounded-full bg-blue-600 flex-shrink-0 flex items-center justify-center text-sm font-black shadow-2xl">1</div>
                  <p className="pt-2 leading-relaxed">Select a marble and jump over its neighbor into an empty hole.</p>
                </div>
                <div className="flex gap-4 items-start">
                  <div className="w-10 h-10 rounded-full bg-blue-600 flex-shrink-0 flex items-center justify-center text-sm font-black shadow-2xl">2</div>
                  <p className="pt-2 leading-relaxed">The jumped marble is vaporized and added to your tray.</p>
                </div>
                <div className="flex gap-4 items-start">
                  <div className="w-10 h-10 rounded-full bg-blue-600 flex-shrink-0 flex items-center justify-center text-sm font-black shadow-2xl">3</div>
              ...
              <button onClick={() => setShowRules(false)} className="mt-10 w-full py-6 btn-3d h-16">
                ...
                <div className="btn-surface bg-blue-600 rounded-[2rem] text-white text-lg font-black uppercase tracking-[0.3em]">Understood</div>
              </button>
           </div>
        </div>
      )}
      ...
      {showLayoutModal && (
        <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 bg-black/98 backdrop-blur-3xl animate-in">
          <div className="relative max-w-lg w-full p-8 rounded-[4rem] bg-slate-950 border border-white/10 text-white shadow-4xl">
                   ...
                   <button key={layout.name} onClick={() => handleLayoutChange(layout)} className="btn-3d w-full h-24 block">
                      ...
                      <div className={`btn-surface flex items-center justify-between px-8 rounded-[2rem] border-2 ${currentLayout.name === layout.name ? 'border-green-400 bg-green-950/40' : 'border-white/5 bg-slate-900'}`}>
                        <div className="text-left">
                          ...
                          <p className="text-[10px] opacity-40 font-black tracking-widest mt-1 uppercase">{layout.description}</p>
                        </div>
                        {currentLayout.name === layout.name && <Check size={28} className="text-green-400" strokeWidth={5} />}
                      </div>
                   </button>
                ))}
             </div>
