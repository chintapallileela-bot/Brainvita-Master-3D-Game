...
export const MoveOverlay: React.FC<MoveOverlayProps> = ({ from, to, theme }) => {
  ...
  useEffect(() => {
    ...
    if (fromEl && toEl) {
      ...
      // Set initial position - matching the "selected" elevated state
      setStyle({
        position: 'absolute',
        left: startLeft,
        top: startTop,
        ...
        transition: 'transform 200ms cubic-bezier(0.175, 0.885, 0.32, 1.275)', // Springy jump
  ...
  return (
    <div style={style}>
       <div className="transform shadow-[0_30px_50px_rgba(0,0,0,0.6)] rounded-full" style={{ transformStyle: 'preserve-3d' }}>
         {/* We pass isSelected=true here just to get the brighter look while jumping */}
